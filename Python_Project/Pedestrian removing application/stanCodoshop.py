...
def solve(images):
    ...
    pixels = []  # create an empty list to store same location of pixels from different images
    for i in range(width):
        for j in range(height):
            for k in range(len(images)):
                pixels.append(images[k].get_pixel(i, j))
            best = get_best_pixel(pixels)
            result.set_pixel(i, j, best)  # add the closest pixel to the blank
            pixels = []
